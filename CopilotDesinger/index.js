...
const group = (group_data, arr) => {
    ...
    const name_div_col = $('<div class="col-5">');
    const qty_reqd_col = $('<div class="col-5">');
    const name_input = input_text()
        .val(group_data.name ?? "")
        .change(function () { group_data.name = $(this).val(); });

    const qty_reqd = input_text()
        .attr('type', 'number')
        .val(String(group_data.qty_reqd ?? 0))
        .change(function () { group_data.qty_reqd = Number($(this).val()); });
    name_div_col.append(name_input);
    qty_reqd_col.append(qty_reqd);
    ...
    name_div_row.append(name_div_col).append(qty_reqd_col).append(move_delete_col);
    ...
    const table = $('<table class="table table-bordered table-condensed table-striped">' +
        ...
        '<tbody></tbody>' +
        '</table>');
    (group_data.opers ?? []).forEach(oper_data => table.find('tbody').append(oper(oper_data, group_data.opers)));
...
$(document).ready(() => {
    ...
    $('#download_json').click(() => {
        ...
        const fileName = data.stage_name + '_' + (data.opers?.map(o => o.name).join('+') ?? '') + '.txt';
